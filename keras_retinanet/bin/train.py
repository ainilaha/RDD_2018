...
from ..preprocessing.rdd import RDDGenerator
...
def create_models(backbone_retinanet, num_classes, weights, multi_gpu=0, freeze_backbone=False, config=None):
    ...
    Args
        backbone_retinanet : A function to call to create a RDD_2018 model with a given backbone.
...
def create_generators(args, preprocess_image):
    ...
    if args.dataset_type == 'coco':
        ...
        train_generator = RDDGenerator(
            args.rdd_path,
            transform_generator=transform_generator,
            images_df=images_df[:-150],
            **common_args
        )
        validation_generator = RDDGenerator(
            args.rdd_path,
            transform_generator=transform_generator,
            images_df=images_df[-500:],
            **common_args
        )
...
def parse_args(args):
    ...
    pascal_parser = subparsers.add_parser('rdd')
    pascal_parser.add_argument('rdd_path', help='Path to dataset directory (ie. /tmp/rdd).')
    ...
    parser.add_argument('--backbone', help='Backbone model used by RDD_2018.', default='resnet152', type=str)
    ...
    parser.add_argument('--steps', help='Number of steps per epoch.', type=int, default=7090) #3545
